...
class ModelDataDeepc(object):
    def __init__(self):
        raise NotImplementedError

    INPUT_NAME = "input"
    INPUT_SHAPE = (1, 240, 420)
...
class ModelDataRefinenet(object):
    def __init__(self):
        raise NotImplementedError
...
def allocate_buffers(engine, deepc: bool, bs: int):
    ...
    if deepc:
        binding_to_type = {"input": np.float32, "output": np.float32, "109": np.float32}
        outbs = bs
    else:
    ...
    for binding in engine:
        size = trt.volume(engine.get_binding_shape(binding)) * outbs #* engine.max_batch_size
